...
{
	int delete = 0;
    return auxDeleteAVL(avl,inf,&delete);
}

AVL *auxDeleteAVL(AVL **avl,Info *inf, int *delete)
{
	AVL *aux;
	if(!(*avl)) return NULL;
	if(!inf) return *avl;
	if(inf->value == rootAVL((*avl))->value){
		if(!(*avl)->left){
			...
			rootAVL(*avl)->value = rootAVL(getLargestElementAVL((*avl)->left))->value;
			(*avl)->left = deleteAVL(&(*avl)->left,rootAVL((*avl)));
		}
		*delete = 1;
	}else if(inf->value > rootAVL((*avl))->value){
		(*avl)->right = auxDeleteAVL(&(*avl)->right,inf,delete);
		if(*delete == 1){
			switch ((*avl)->balance)
			{
				case 1:
		...
		(*avl)->left = auxDeleteAVL(&(*avl)->left,inf,delete);
		if(*delete == 1){
			switch ((*avl)->balance)
			{
				case 1:
...
AVL *getLargestElementAVL(AVL *avl){
	if(!avl->right) return avl;
	return getLargestElementAVL(avl->right);
}
